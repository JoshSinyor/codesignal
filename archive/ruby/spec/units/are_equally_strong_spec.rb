...
describe 'equally_strong?' do
  it 'passes the supplied tests with 4 inputs and 1 output' do
    ARE_EQUALLY_STRONG_TEST_I_O.each do |element|
      puts "  Executing test #{element[:test]}..."
      expect(equally_strong?(element[:input1], element[:input2], element[:input3],
                             element[:input4])).to eq(element[:output])
      puts '    Passed.'
    end
  end
end
