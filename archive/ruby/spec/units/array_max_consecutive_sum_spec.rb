...
ARRAY_MAX_CONSECUTIVE_SUM_TEST_I_O = [
  ...
  { test: 7, import: false, input1: [963,
                                     741,
                                     22,
                                     851,
                                     399,
                                     ...
                                     784,
                                     607,
                                     341], input2: 23, output: 14_232 }, # 07
...
describe 'array_max_consecutive_sum' do
  it 'passes the supplied tests with 2 inputs and 1 output' do
    start_time = Time.new
    repetitions = 1
    repetitions.times do
      ARRAY_MAX_CONSECUTIVE_SUM_TEST_I_O.each do |element|
        case element[:import]
        when 'json'
        ...
        expect(array_max_consecutive_sum(element[:input1], element[:input2])).to eq(element[:output])
        puts '    Passed.'
      end
    end
